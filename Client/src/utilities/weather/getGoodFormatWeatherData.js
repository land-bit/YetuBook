...
const getGoodFormatWeatherData = async () => {
  ...
  let byHour = [],
    currDt = prevision.list[0].dt_txt.slice(8, 10),
    next5Days = [];
  ...
  for (let i = 0; i < 6; i++) {
    byHour.push({
      ...
      desc: formatUtils.capitalizeFirstLetters(
        prevision.list[i].weather[0].description
      ),
      ...
      windChill: formatUtils.calculateWindChill(
        prevision.list[i].main.temp,
        prevision.list[i].wind.speed
      ),
      dewPoint: formatUtils.calculateWindChill(
        prevision.list[i].main.temp,
        prevision.list[i].main.humidity
      ),
      uvi: formatUtils.calculateUVI(
        prevision.city.coord.lat,
        1500,
        parseInt(prevision.list[i].dt_txt.split(" ")[1].split(":")[0]),
        prevision.list[i].clouds.all
      ),
    });
  }
  ...
  for (let i = 0; i < prevision.list.length; i++) {
    ...
    next5Days.push({
      ...
      uvi: formatUtils.calculateUVI(
        prevision.city.coord.lat,
        1500,
        prevision.list[i].dt_txt.split(" ")[1].split(":")[0],
        prevision.list[i].clouds.all
      ),
    });
  }
  ...
  return {
    ...
    currentWeather: {
      ...
      dewPoint: formatUtils.calculateDewPoint(
      ...
      uvi: formatUtils.calculateUVI(
        ...
        1500,
        formatUtils.getFormattedTodayHour().split(":")[0],
    ...
    currentHour: {
      ...
      desciption: formatUtils.capitalizeFirstLetters(
