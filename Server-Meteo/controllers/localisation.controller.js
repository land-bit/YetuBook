...
export async function getLocalisationHistory(req, res) {
  ...
  if (!date) {
    return res
      .status(400)
      .json({ error: "Date manquante, veuillez fournir la date" });
  }
  ...
  if (match) {
    ...
    console.log(
      "Format de date non supporté voici un expemple du bon format '30-Juillet-2024' "
    );
  }
  ...
  try {
    ...
    if (!localisation[0]) {
      return res
        .status(404)
        .json({ error: "Données de pollution non trouvées pour cette date" });
    }

    // Envoi de la réponse avec les données de pollution
    res.status(200).json(localisation[0]);
  } catch (error) {
    console.error(error);
...
export async function updateLocalisationHistory(req, res) {}

export async function deleteLocalisationHistory(req, res) {}
