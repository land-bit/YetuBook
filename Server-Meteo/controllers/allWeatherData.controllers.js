...
export async function getAllWeatherData(req, res) {
  ...
  try {
    ...
    const newAirPollution = await db.airPollution.create({
      data: airPollution,
    });
    const newCurrentWeather = await db.currentWeather.create({
      data: currentWeather,
    });
  ...
  try {
    ...
    if (
      ...
      return res
        .status(404)
        .json({
          error: "Données météorologiques non trouvées pour cette date",
        });
    }
